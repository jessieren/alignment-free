...
vector<double> D2C2computeLongseq(int ZI, int k, KMERINFO* speciesKmerInfoA, KMERINFO* speciesKmerInfoB)
{
  ...
  for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++)
  {
    ...
    for(int speciesID=0; speciesID<2; speciesID++)
    {
      ...
      p_w[speciesID] = speciesKmerInfo->HashPw[currentKmerTen];
      EX_w[speciesID] = SciMultiple(p_w[speciesID], speciesKmerInfo->totalKmer);
      X_w[speciesID] = TransToScientific(speciesKmerInfo->HashTable[currentKmerTen]);
...
vector<double> D2C2computeNGSFreq(int ZI, int k, KMERINFO* speciesKmerInfoA, KMERINFO* speciesKmerInfoB)
{
	// compute D2 statistics
	SCIENTIFIC_NUMBER D2, D2star, D2shepp;
	D2.value = 0; D2.factor = 0;
	...
	for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++)
	{
		...
		//SCIENTIFIC_NUMBER p_w[2], EX_w[2], X_w[2], X_w_tilde[2], X_w_tilde_var[2];
		SCIENTIFIC_NUMBER   p_w[2], X_w[2],  f_w[2], f_w_tilde[2], f_w_tilde_var[2];
		...
		for(int speciesID=0; speciesID<2; speciesID++)
		{
			...
			//EX_w[speciesID] = SciMultiple(p_w[speciesID], speciesKmerInfo->totalKmer);
			X_w[speciesID] = TransToScientific(speciesKmerInfo->HashTable[currentKmerTen] + speciesKmerInfo->HashTable[currentKmerRevTen]);
			f_w[speciesID] = SciMultiple(X_w[speciesID], SciInverse(TransToScientific(speciesKmerInfo->totalKmer)));
			//cout << "Species" << speciesID << " Ten:" << currentKmerTen << " Four: " ;
			//printFour(currentKmerFour);
			//cout << "forward:" << HashTable[speciesID][currentKmerTen] << " reverse:" << HashTable[speciesID][currentKmerRevTen] << TransToReal(X_w[speciesID]) << endl;
			//X_w_tilde[speciesID] = SciAddition(X_w[speciesID], SciNegative(EX_w[speciesID]));
			f_w_tilde[speciesID] = SciAddition(f_w[speciesID], SciNegative(p_w[speciesID]));
			
			//cout << TransToReal(p_w[speciesID]) << ", " << TransToReal(EX_w[speciesID]) << ", " << TransToReal(X_w[speciesID]) << endl;
			
			// prepare for C2 compute
			C2_below[speciesID] = SciAddition(C2_below[speciesID], SciPow(f_w[speciesID], 2));
			// prepare for C2star compute
			// 20140910: the denominator cannot be 0
			if(p_w[speciesID].value != 0)
			{
				f_w_tilde_var[speciesID] = SciMultiple(f_w_tilde[speciesID], SciInverse(SciPow(p_w[speciesID],0.5)));
				C2star_below[speciesID] = SciAddition(C2star_below[speciesID], SciPow(f_w_tilde_var[speciesID], 2));
			}
		...
		//cout << "D2:" << D2.value << ", " << D2.factor << endl;
		//if(D2.value == 0.0){cout << "D2.value = 0.0" << endl;}
		D2 = SciAddition(SciMultiple(f_w[0], f_w[1]), D2);
		//cout << "X_w[0]:" << TransToReal(X_w[1]) << " X_w[1]:" << TransToReal(X_w[1]) << " D2:" << TransToReal(D2) << endl;
		
		SCIENTIFIC_NUMBER D2star_above = SciMultiple(f_w_tilde[0], f_w_tilde[1]);
		SCIENTIFIC_NUMBER D2star_below = SciPow(SciMultiple(p_w[0], p_w[1]), 0.5);
		if(D2star_below.value != 0)
		{
			D2star = SciAddition(D2star, SciMultiple(D2star_above, SciInverse(D2star_below)));
		}
		
		SCIENTIFIC_NUMBER D2shepp_below = SciPow(SciAddition(SciPow(f_w_tilde[0],2), SciPow(f_w_tilde[1],2)),0.5);
		// 20140910: the denominator cannot be 0
		if(D2shepp_below.value != 0)
		{
			D2shepp = SciAddition(D2shepp, SciMultiple(D2star_above, SciInverse(D2shepp_below)));
			// prepare for c2shepp compute
			C2shepp_below[0] = SciAddition(C2shepp_below[0], SciMultiple(SciPow(f_w_tilde[0],2), SciInverse(D2shepp_below)));
			C2shepp_below[1] = SciAddition(C2shepp_below[1], SciMultiple(SciPow(f_w_tilde[1],2), SciInverse(D2shepp_below)));
		}
...
vector<double> EuMaChCombineDistNGS(int ZI, int k, KMERINFO* speciesKmerInfoA, KMERINFO* speciesKmerInfoB)
{
  ...
  for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++)
  {
    ...
    if( TransToReal(diff) > TransToReal(ChDist) )
    {
      ChDist = diff;
		...
		// uniq, 20150903
		// frequency
		SCIENTIFIC_NUMBER XU1; XU1.value=0; XU1.factor=0;
		SCIENTIFIC_NUMBER XU2; XU2.value=0; XU2.factor=0;
		if(  currentKmerRevTen != currentKmerTen )
		{
			// the case of AA - TT
			XU1 = TransToScientific( ( speciesKmerInfoA->HashTable[currentKmerTen] + speciesKmerInfoA->HashTable[currentKmerRevTen] ) / double ( speciesKmerInfoA->totalKmer ) );
			XU2 = TransToScientific( ( speciesKmerInfoB->HashTable[currentKmerTen] + speciesKmerInfoB->HashTable[currentKmerRevTen] ) / double ( speciesKmerInfoB->totalKmer ) );
		}else{
			// the case of AT - AT
			XU1 = TransToScientific( ( speciesKmerInfoA->HashTable[currentKmerTen] ) / double ( speciesKmerInfoA->totalKmer ) );
			XU2 = TransToScientific( ( speciesKmerInfoB->HashTable[currentKmerTen] ) / double ( speciesKmerInfoB->totalKmer ) );
		}
		
		SCIENTIFIC_NUMBER diffU = SciAddition(XU1, SciNegative(XU2)) ;
		diffU.value = fabs(diffU.value);
		if( currentKmerRevTen >= currentKmerTen )
		{
			
			EuUDist = SciAddition( EuUDist, SciPow( diffU , 2 ) );
			
			MaUDist = SciAddition( MaUDist, diffU ) ;
			
			if( TransToReal(diffU) > TransToReal(ChUDist) )
			{
				ChUDist = diffU;
			}
			count++;
  ...
  EuMaChCombineDistvalues.push_back(TransToReal(ChDist));
	...
	EuMaChCombineDistvalues.push_back(TransToReal(SciPow(EuUDist, 0.5)));
	EuMaChCombineDistvalues.push_back(TransToReal(MaUDist));
	EuMaChCombineDistvalues.push_back(TransToReal(ChUDist));
...
vector<double> HAOTeelingcompute(int ZI, int k, KMERINFO* speciesKmerInfoA, KMERINFO* speciesKmerInfoB)
{
  ...
  //cout << HAOvalue << endl;
	...
	HaoTeelingvalues.push_back(HAOvalue);
	HaoTeelingvalues.push_back(TeelingValue);
...
void computeMultiStats(int ZI, int k, KMERINFO* speciesKmerInfoA, KMERINFO* speciesKmerInfoB, SPECIESINFO speciesInfoA, SPECIESINFO speciesInfoB)
{
	
	// 1. compute the D2 statistics
	string D2StatNames[18] = {"D2", "d2", "D2star", "d2star", "D2shepp", "d2shepp", "D2NGS", "d2NGS", "D2starNGS", "d2starNGS", "D2sheppNGS", "d2sheppNGS", "D2NGSFreq", "d2NGSFreq", "D2starNGSFreq", "d2starNGSFreq", "D2sheppNGSFreq", "d2sheppNGSFreq"} ;
	...
	vector<double> D2C2NGSvalues = D2C2computeNGSFreq(ZI, k, speciesKmerInfoA, speciesKmerInfoB);
	...
	for(int i = 0; i < D2C2NGSvalues.size(); i++)
	{
		...
		cout << D2StatNames[i+12] << ", " << D2C2NGSvalues[i] << endl ;
	}
...
////
////		//"; " << speciesKmerInfoB->HashTableK_1[currentKmerRemoveFirstTen] << " totalK_1 " << speciesKmerInfo->totalK_1 << "; " << speciesKmerInfo->HashTableK_2[currentKmerRemoveTwoTen] << " totalK_2 " << speciesKmerInfo->totalK_2[speciesID] << endl;
//		
...
//
//	cout << "EuU, " << EuMaChCombineDistvalues[3] << endl ;
//	cout << "MaU, " << EuMaChCombineDistvalues[4] << endl ;
//	cout << "ChU, " << EuMaChCombineDistvalues[5] << endl ;
//	
//	cout << "EuF, " << EuMaChCombineDistvalues[6] << endl ;
