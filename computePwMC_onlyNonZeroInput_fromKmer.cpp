...
void pwIIDwrite(int ZI, int k, int speciesID, ofstream& pwIIDfout)
{
  ...
  for(unsigned long ten = 0; ten < pow(ZI, k); ten++ )
  {
    ...
    // write the HashPw into file
    if(ten % ZI == (ZI-1)){
      pwIIDfout << TransToReal(pw) << endl;
      if(ten != pow(ZI, k)-1){
        pwIIDfout << (ten/ZI + 1) << ",";
      }
    }else{
      if(ten == 0){ pwIIDfout << ten << ","; }
      pwIIDfout << TransToReal(pw) << ",";
...
void pwMCwrite(int ZI, int k, int order, int speciesID, ofstream& pwMCfout)
{
  ...
  for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++ )
  {
    ...
    // write the HashPw into file
    if(currentKmerTen % ZI == (ZI-1)){
      pwMCfout << TransToReal(pw) << endl;
      if(currentKmerTen != pow(ZI, k)-1){
        pwMCfout << (currentKmerTen/ZI + 1) << ",";
      }
    }else{
      if(currentKmerTen == 0){ pwMCfout << currentKmerTen << ","; }
      pwMCfout << TransToReal(pw) << ",";
...
vector<double> D2C2compute(int ZI, int k)
{
...
//  sprintf(order1str, "%d", (order+1));
  ...
  for(int speciesID=0; speciesID<1; speciesID++)
  {
    ...
    if(order[speciesID] == 0)
    {
      ...
      strcpy(kmerFileOrder1PathName, dirPath);
      strcat(kmerFileOrder1PathName, "/");
      ...
      strcat(kmerFileOrder1PathName, "_d0_singleStrand_wordcount");
      ...
      char pwIIDoutputfile[1000];
      strcpy(pwIIDoutputfile, dirPath);
      strcat(pwIIDoutputfile, "/");     
      strcat(pwIIDoutputfile, speciesName[speciesID]);
      strcat(pwIIDoutputfile, "_k");
      strcat(pwIIDoutputfile,kstr);
      strcat(pwIIDoutputfile,"_order");
      strcat(pwIIDoutputfile,orderstr);
      strcat(pwIIDoutputfile,"_singleStrand_pwMC");
      ofstream pwIIDfout(pwIIDoutputfile);
      pwIIDwrite(ZI, k, speciesID, pwIIDfout);
      pwIIDfout.close();
      ...
      strcpy(kmerFileOrderPathName, dirPath);
      strcat(kmerFileOrderPathName, "/");
      strcat(kmerFileOrderPathName, speciesName[speciesID]);
      strcat(kmerFileOrderPathName, "_k");
      strcat(kmerFileOrderPathName, orderstr);
      strcat(kmerFileOrderPathName, "_d0_singleStrand_wordcount");
      ...
      // compute the pw
      cout << "== compute the pw for the words == " << endl;
      // write the hashPw into file while computing it
      char pwMCoutputfile[1000];
      strcpy(pwMCoutputfile, dirPath);
      strcat(pwMCoutputfile, "/");     
      strcat(pwMCoutputfile, speciesName[speciesID]);
      strcat(pwMCoutputfile, "_k");
      strcat(pwMCoutputfile,kstr);
      strcat(pwMCoutputfile,"_order");
      strcat(pwMCoutputfile,orderstr);
      strcat(pwMCoutputfile,"_singleStrand_pwMC");
      ofstream pwMCfout(pwMCoutputfile);
      pwMCwrite(ZI, k, order[speciesID], speciesID, pwMCfout);
      pwMCfout.close();
    }
