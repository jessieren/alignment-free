...
void loadKmerCountHash(char *currentKmerFilePathName, string kmerUsage)
{
  ...
  { 
    ...
    if(kmerUsage == "kmerCount"){
      HashTable[currentKmerID] = currentKmerCount;
      totalKmer = totalKmer + currentKmerCount;
      //cout << "kmerCount " << currentKmerCount << endl;
    }else if(kmerUsage == "kmerCount_1"){
      HashTableK_1[currentKmerID] = currentKmerCount;
      totalK_1 = totalK_1 + currentKmerCount;
      //cout << "kmerOrder " << currentKmerCount << endl;
    }else if(kmerUsage == "kmerCount_2"){
      HashTableK_2[currentKmerID] = currentKmerCount;
      totalK_2 = totalK_2 + currentKmerCount;
      //cout << "kmerOrder " << currentKmerCount << endl;
    }else if(kmerUsage == "kmerOrder"){
      HashTableOrder[currentKmerID] = currentKmerCount;
      totalOrder = totalOrder + currentKmerCount;
      //cout << "kmerOrder " << currentKmerCount << endl;
    }else if(kmerUsage == "kmerOrder+1"){
      HashTableOrder_1[currentKmerID] = currentKmerCount;
      totalOrder_1 = totalOrder_1 + currentKmerCount;
...
void pwIID(int ZI, int k)
{
  ...
  { 
    ...
    probIID[index] = TransToScientific(HashTableOrder_1[index]/double(totalOrder_1));
  }
...
void pwMC(int ZI, int k, int order, vector<SCIENTIFIC_NUMBER>& iniProb, vector< vector<SCIENTIFIC_NUMBER> > &transMatrix)
{
  ...
  for(int currentRow = 0; currentRow < transRowSize; currentRow++)
  {
    ...
    unsigned long countBelow = HashTableOrder[currentRow];
    double probBelow = countBelow/double(totalOrder);
    ...
    for(int currentCol = 0; currentCol < transColSize; currentCol++)
    {
      ...
      unsigned long countAbove = HashTableOrder_1[currentRowColTen];
      double probAbove = countAbove/double(totalOrder_1);
  ...
  for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++ )
  {
    ...
    HashPw[currentKmerTen] = pw;
		//cout << currentKmerTen << endl;
    //printFour(currentKmerFour);
...
void printD2Feature(int ZI, int k, int order, char *outputFileDir, char *shortName)
{
	//cout << "test2" << endl;
	vector<SCIENTIFIC_NUMBER> X_w_tilde_var;
	SCIENTIFIC_NUMBER X_w_tilde_var_norm; /// initialization is a must!! 20151027
	X_w_tilde_var_norm.factor=0; X_w_tilde_var_norm.value=0;
	...
	for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++)
  {
    ...
    SCIENTIFIC_NUMBER p_w, EX_w, X_w, X_w_tilde;
		
		p_w = SciAddition(HashPw[currentKmerTen], HashPw[currentKmerRevTen]);
		EX_w = SciMultiple(p_w, totalKmer);
		X_w = TransToScientific(HashTable[currentKmerTen] + HashTable[currentKmerRevTen]);
		...
		X_w_tilde = SciAddition(X_w, SciNegative(EX_w));
		...
		//cout << "X_w_tilde" << TransToReal(X_w_tilde) << endl;
		//cout << "X_w_tilde_var.back()" << TransToReal(X_w_tilde_var.back()) << endl;
		
  }
	
	//cout << "X_w_tilde_var_norm" << TransToReal(X_w_tilde_var_norm) << endl;
	// print
	//cout << "test3" << endl;
...
void printEuFeature(int ZI, int k, int order, char *outputFileDir, char *shortName)
{
	...
	for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++)
	{
		...
		SCIENTIFIC_NUMBER X_w = TransToScientific( (HashTable[currentKmerTen] + HashTable[currentKmerRevTen])/double(2 * totalKmer));
		
		
		cout << currentKmerTen << "," << TransToReal(X_w) << endl;
...
int main(int argc, char **argv)   //EDIT main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		switch (c)
		{
			...
			case 'o':
				...
				//printf ("option -o, the output feature file directory, with value `%s'\n", optarg);
				break;
  ...
  // load the kmer count hashtables of the two dataset
	...
	strcpy(kmerFilePathName, inputFileDir);
	//strcat(kmerFilePathName, "kmerCount/");
	strcat(kmerFilePathName, speciesName);
