...
// find the reverse complimentary of a word. word composed of 0, 1, 2, 3
...
// compute the probs of ZI^k words under i.i.d model
...
// compute the probs of ZI^k words under MC model
...
// compute D2C2 using single strand kmer count
...
// compute D2C2 using double strands kmer count
// the kmer count for reverse complimentary words can be found in the single strand kmer count file
...
// compute Eu Ma Ch distances using double strands
...
// compute Eu Ma Ch distances using doule strands, and consider only unique kmer-reverse complimentary word pairs
vector<double> EuMaChDistUniqNGS(int ZI, int k)
{
	//cout << TransToReal(D2) << endl;
...
// compute EuF: normalize Eu by the expected word count
...
// compute Hao similarity
/// need to write a version of using double strand
...
// compute Jensen–Shannon divergence under i.i.d model
...
// compute Jensen–Shannon divergence under MC model
...
int main(int argc, char **argv)   //EDIT main(int argc, char *argv[])
{
  ...
  //fout << "Ch, " << EuMaChDistvalues[2] << endl ;
	...
	vector<double> EuMaChUDistvalues = EuMaChDistUniqNGS(ZI, k) ;
