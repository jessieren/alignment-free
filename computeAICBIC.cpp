...
vector<double> modifiedEuDistNGS(int ZI, int k)
{
	...
	for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++)
	{
		...
		SCIENTIFIC_NUMBER Fw1 = SciMultiple( Nw1, SciInverse(Ew1) );
		...
		SCIENTIFIC_NUMBER Fw2 = SciMultiple( Nw2, SciInverse(Ew2));
...
// AIC/BIC under r-th order MC
vector<double> AICBICcompute(int ZI, int order, int speciesID)
{
	double AIC = 2 * (ZI-1) * pow(ZI, order) ;
	double BIC = (ZI-1) * pow(ZI, order) * log( totalOrder_1[speciesID] ) ;
	
	//cout << AIC << ", " << BIC << ", " << AICd << ", " << BICd << endl;
	
	SCIENTIFIC_NUMBER logLH;
	logLH.value = 0; logLH.factor = 0 ;
	
	for(unsigned long currentOrderPlus1merTen = 0; currentOrderPlus1merTen < pow(ZI, (order+1)); currentOrderPlus1merTen++)
	{
		vector<int> currentOrderPlus1merFour = ten2four(currentOrderPlus1merTen, (order+1));
		
		if( order > 0 )
		{
			// remove the last one
			vector<int> current_num_1_order_Four = currentOrderPlus1merFour;
			current_num_1_order_Four.erase( current_num_1_order_Four.end() - 1) ;
			unsigned long current_num_1_order_ten = four2ten(current_num_1_order_Four, current_num_1_order_Four.size());
			
			
			if( HashTableOrder[speciesID][current_num_1_order_ten] != 0 & HashTableOrder_1[speciesID][currentOrderPlus1merTen] != 0){
				logLH = SciAddition(logLH, ( double(HashTableOrder_1[speciesID][currentOrderPlus1merTen])
																		* log ( double(HashTableOrder_1[speciesID][currentOrderPlus1merTen]) / double(HashTableOrder[speciesID][current_num_1_order_ten]) ) ) );
				
				//cout << ( double(HashTableOrder_1[speciesID][currentKPlus1merTen]) * log ( double(HashTableOrder_1[speciesID][currentKPlus1merTen]) / double(HashTableOrder[speciesID][current_num_1_k_ten]) ) )  << endl;
				//cout << TransToReal(logLH) << " (k+1)mer " << HashTableOrder_1[speciesID][currentKPlus1merTen] << " kmer " << HashTableOrder[speciesID][current_num_1_k_ten] << endl;
				
			}
		}else{
			
			// order = 0
			if(HashTableOrder_1[speciesID][currentOrderPlus1merTen] != 0 & totalOrder_1[speciesID] != 0)
			{
				logLH = SciAddition(logLH, ( double(HashTableOrder_1[speciesID][currentOrderPlus1merTen])
																		* log ( double(HashTableOrder_1[speciesID][currentOrderPlus1merTen]) / totalOrder_1[speciesID] ) ) ) ;
			}
   
		}
	}
	//cout << " value " << logLH.value << " factor " << logLH.factor << endl;
	AIC = -2 * TransToReal(logLH) + AIC ;
	BIC = -2 * TransToReal(logLH) + BIC ;
	
	vector<double> AICBICvalues;
	AICBICvalues.push_back(AIC);
	AICBICvalues.push_back(BIC);
	return AICBICvalues;
...
int main(int argc, char **argv)   //EDIT main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		c = getopt_long (argc, argv, "a:b:i:o:",
...
//				break;
	...
	if (inputFileDir != NULL) {
		std::string inputPath = inputFileDir;
  ...
  for(int speciesID=0; speciesID<1; speciesID++)
  {
...
//      strcpy(kmerFilek_1PathName, inputFileDir);
...
//      strcpy(kmerFilek_2PathName, inputFileDir);
